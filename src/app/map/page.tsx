...
function MapPageInner() {
  ...
  useEffect(() => {
    ...
    // High accuracy OFF → 빠르게 잡히도록, timeout 15초
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        console.warn('GPS 실패, 기본 위치 사용:', err.message)
        setUserLocation({ lat: 35.5384, lng: 129.3114 })
        setLocationStatus('fallback')
      },
      ...
      fetchNearbyStores(lat, lng, '전체').then((results) => {
        if (targetName) {
          ...
          if (found) setSelectedStore(found)
            ...
            id: `saved-${Date.now()}`, name: decoded, category: '저장한 가게',
  ...
  useEffect(() => {
    ...
    stores.forEach((store) => {
      ...
      const score = storeScores[store.id] || store.rating

      let bg = 'white', color = '#1A1A1A', border = '#ddd', shadow = '0 2px 6px rgba(0,0,0,0.15)'
      if (isSelected) { bg = '#1A1A1A'; color = 'white'; border = '#1A1A1A'; shadow = '0 4px 12px rgba(0,0,0,0.4)' }
      else if (isSaved) { bg = '#FF5A3D'; color = 'white'; border = '#FF5A3D'; shadow = '0 2px 8px rgba(255,90,61,0.4)' }
      else if (isSearch) { bg = '#4A90E2'; color = 'white'; border = '#4A90E2'; shadow = '0 2px 8px rgba(74,144,226,0.4)' }

      const ratingHtml = score
        ? `<div style="font-size:10px;font-weight:600;color:${isSelected || isSaved || isSearch ? 'rgba(255,255,255,0.85)' : '#FF9500'};margin-top:1px;">⭐ ${Number(score).toFixed(1)}</div>`
        : ''
  ...
  const loadStoreScores = async () => {
    const { data } = await supabase.from('stores').select('id, user_score, editor_score')
    if (!data) return
    const scores: Record<string, number> = {}
    data.forEach((s: any) => {
      if (s.user_score) scores[String(s.id)] = s.user_score
    })
    ...
    try {
      ...
      if (data.documents) {
        ...
        if (results.length && kakaoMapRef.current) {
          kakaoMapRef.current.setCenter(new window.kakao.maps.LatLng(results[0].latitude, results[0].longitude))
        }
      }
  ...
  const handleSave = async () => {
    ...
    if (isSaved) {
      await supabase.from('saved_stores').delete().eq('user_id', user.id).eq('store_id', storeIdStr)
  ...
  // 상세보기: 카카오 place_url 또는 카카오맵 검색 URL로 이동
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', fontFamily: 'Pretendard, -apple-system, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: 'white', borderBottom: '1px solid #F2F2F2', zIndex: 10 }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {locationStatus === 'loading' && <span style={{ fontSize: '11px', color: '#999', background: '#F2F2F2', padding: '4px 8px', borderRadius: '10px' }}>📍 위치 확인 중...</span>}
          {locationStatus === 'fallback' && <span style={{ fontSize: '11px', color: '#FF9500', background: '#FFF3E0', padding: '4px 8px', borderRadius: '10px' }}>⚠️ 기본위치</span>}
          {locationStatus === 'success' && <span style={{ fontSize: '11px', color: '#34C759', background: '#E8F9EE', padding: '4px 8px', borderRadius: '10px' }}>📍 위치확인</span>}
      ...
      <div style={{ padding: '10px 16px', background: 'white', borderBottom: '1px solid #F2F2F2', display: 'flex', gap: '8px' }}>
        ...
        <button onClick={searchByKeyword}
          style={{ background: '#FF5A3D', color: 'white', border: 'none', borderRadius: '12px', padding: '10px 16px', fontSize: '14px', fontWeight: '700', cursor: 'pointer' }}>검색</button>
        {isSearchMode && (
          <button onClick={() => { setSearchKeyword(''); setIsSearchMode(false); if (userLocation) fetchNearbyStores(userLocation.lat, userLocation.lng, activeCategory) }}
            style={{ background: '#F2F2F2', color: '#666', border: 'none', borderRadius: '12px', padding: '10px 12px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>✕</button>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', padding: '8px 16px', overflowX: 'auto', background: 'white', borderBottom: '1px solid #F2F2F2' }}>
        {CATEGORIES.map((cat) => (
          <button key={cat} onClick={() => { setActiveCategory(cat); if (userLocation) fetchNearbyStores(userLocation.lat, userLocation.lng, cat) }}
        ...
        <div style={{ background: 'white', borderRadius: '20px 20px 0 0', padding: '16px 16px 8px', boxShadow: '0 -4px 20px rgba(0,0,0,0.1)', maxHeight: '50vh', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
            <div style={{ flex: 1 }}>
              ...
              {selectedStore.address && <p style={{ color: '#888', fontSize: '13px', margin: '0 0 2px' }}>📍 {selectedStore.address}</p>}
              {selectedStore.phone && <p style={{ color: '#888', fontSize: '13px', margin: 0 }}>📞 {selectedStore.phone}</p>}
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginTop: '8px' }}>
            ...
            <button onClick={() => handleDetail(selectedStore)}
              style={{ background: '#F8F8F8', color: '#1A1A1A', border: '1px solid #E0E0E0', borderRadius: '12px', padding: '11px 8px', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>
              🔍 상세보기
            </button>
          </div>
          {selectedStore.placeUrl && (
            <p style={{ fontSize: '11px', color: '#bbb', textAlign: 'center', margin: '8px 0 0' }}>카카오맵에서 메뉴·영업시간 확인 가능</p>
          )}
        </div>
      )}
...
export default function MapPage() {
  return (
    <Suspense fallback={<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', fontSize: '16px', color: '#FF5A3D' }}>🗺️ 지도 불러오는 중...</div>}>
      <MapPageInner />
    </Suspense>
  )
}
