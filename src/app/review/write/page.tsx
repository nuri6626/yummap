...
const storePhone = decodeURIComponent(searchParams.get('store_phone') || '')
    ...
    <div style={{ display: 'flex', gap: '6px' }}>
      {[1, 2, 3, 4, 5].map(n => (
        <button key={n} onClick={() => onClick(n)}
          style={{ width: '36px', height: '36px', borderRadius: '50%', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: '700', background: n <= current ? '#FF5A3D' : '#F2F2F2', color: n <= current ? 'white' : '#999', transition: 'all 0.2s' }}>
          {n}
        </button>
      ))}
    </div>
        ...
        if (existingStore) {
          ...
          const { data: newStore, error: storeError } = await supabase
            ...
            .insert({
              kakao_id: storeId,
              name: storeName,
              category: searchParams.get('store_category') || '음식점',
              address: searchParams.get('store_address') || '',
              latitude: parseFloat(searchParams.get('store_lat') || '0'),
              longitude: parseFloat(searchParams.get('store_lng') || '0'),
              phone: searchParams.get('store_phone') || '',
            })
            .select('id')
            .single()
  ...
  {storeName || '지도에서 가게를 선택해주세요'}
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: '16px', marginBottom: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          ...
          <input type="text" placeholder="예: 짬뽕, 탕수육..." value={menuName}
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: '16px', marginBottom: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          ...
          <textarea placeholder="맛, 분위기, 서비스 등 자유롭게 작성해주세요..." value={content}
