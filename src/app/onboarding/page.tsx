...
export default function OnboardingPage() {
  ...
  <img src="/yum2.png" style={{ height:'32px' }} />
      ...
      <div style={{ background:'white', padding:'16px 20px 0' }}>
        ...
        <div style={{ background:'#F2F2F2', borderRadius:'99px', height:'6px', marginBottom:'0' }}>
          <div style={{ background:'linear-gradient(90deg, #FF5A3D, #FF8560)', height:'6px', borderRadius:'99px', width:`${(step/3)*100}%`, transition:'width 0.4s' }}/>
        </div>
      </div>
        ...
        {step === 1 && (
          <div style={{ background:'white', borderRadius:'24px', padding:'28px 24px', boxShadow:'0 4px 16px rgba(0,0,0,0.06)' }}>
            <div style={{ textAlign:'center', marginBottom:'28px' }}>
              ...
              <h2 style={{ fontSize:'22px', fontWeight:'800', color:'#1A1A1A', margin:'0 0 8px', letterSpacing:'-0.5px' }}>
                어떻게 불러드릴까요?
              </h2>
            ...
            <input
              ...
              maxLength={12}
              style={{ width:'100%', padding:'14px 16px', borderRadius:'14px', border:'2px solid #F2F2F2', fontSize:'15px', fontWeight:'600', outline:'none', boxSizing:'border-box', background:'#FAFAFA', color:'#1A1A1A' }}
              onFocus={e => e.target.style.borderColor = '#FF5A3D'}
              onBlur={e => e.target.style.borderColor = '#F2F2F2'}
            />
            <p style={{ textAlign:'right', fontSize:'12px', color:'#ccc', marginTop:'6px' }}>{nickname.length}/12</p>
            <button
              onClick={() => nickname.trim() ? setStep(2) : alert('닉네임을 입력해주세요')}
              style={{ width:'100%', background:'linear-gradient(135deg, #FF5A3D, #FF8560)', color:'white', fontWeight:'800', fontSize:'16px', padding:'15px', borderRadius:'16px', border:'none', cursor:'pointer', marginTop:'8px', boxShadow:'0 4px 16px rgba(255,90,61,0.35)' }}
            >
              다음 →
            </button>
          </div>
          ...
          <div style={{ background:'white', borderRadius:'24px', padding:'28px 24px', boxShadow:'0 4px 16px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:'10px', marginBottom:'24px' }}>
              {CUISINES.map(c => (
                <button
                  ...
                  style={{
                    padding:'10px 18px', borderRadius:'99px', fontSize:'14px', fontWeight:'700', border:'none', cursor:'pointer', transition:'all 0.2s',
                    background: selectedCuisines.includes(c) ? 'linear-gradient(135deg, #FF5A3D, #FF8560)' : '#F2F2F2',
                    color: selectedCuisines.includes(c) ? 'white' : '#666',
                    boxShadow: selectedCuisines.includes(c) ? '0 4px 12px rgba(255,90,61,0.3)' : 'none',
                  }}
                >
                  {c}
                </button>
            ...
            <div style={{ display:'flex', gap:'10px' }}>
              ...
              <button
                ...
                disabled={loading}
                style={{ flex:2, background: loading ? '#ccc' : 'linear-gradient(135deg, #FF5A3D, #FF8560)', color:'white', fontWeight:'800', fontSize:'15px', padding:'14px', borderRadius:'16px', border:'none', cursor: loading ? 'not-allowed' : 'pointer', boxShadow: loading ? 'none' : '0 4px 16px rgba(255,90,61,0.35)' }}
              >
                {loading ? '저장 중...' : '완료! 🎉'}
              </button>
            </div>
